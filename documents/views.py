...
from reminders.models import ReminderLog, ReminderRule
...
class CustomerDocumentUpdateView(BrokerRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        # Vor dem Speichern alten Status merken, um Statuswechsel auf "sent" zu erkennen.
        old_status = self.object.status
        response = super().form_valid(form)
        new_status = self.object.status

        # Nur wenn der Status neu auf "sent" gesetzt wurde, ReminderLogs erzeugen.
        if (
            old_status != CustomerDocument.Status.SENT
            and new_status == CustomerDocument.Status.SENT
        ):
            if not self.object.sent_to_customer_at:
                self.object.sent_to_customer_at = timezone.now()
                self.object.save(update_fields=["sent_to_customer_at"])

            rules = ReminderRule.objects.filter(
                broker=self.object.broker,
                trigger_event=ReminderRule.TriggerEvent.DOCUMENT_SENT,
                enabled=True,
            )

            for rule in rules:
                # Pro Dokument/Regel nur einen Reminder anlegen.
                already_exists = ReminderLog.objects.filter(
                    broker=self.object.broker,
                ...
                due_at = self.object.sent_to_customer_at + timedelta(
                    days=rule.days_after
                )
